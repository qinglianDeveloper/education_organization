  ...
  methods: {
    ...
    submit() {
      ...
      if (this.form.organizationId === "")
        return this.$Message.error("1.所在培训机构ID不能为空");
      if (this.form.identity === "")
        return this.$Message.error("2.身份不能为空");
      if (this.form.name === "") return this.$Message.error("5.姓名不能为空");
      if (this.form.gender === "") return this.$Message.error("6.性别不能为空");
      if (this.form.birthday === "")
        return this.$Message.error("7.出生日期不能为空");
      if (this.form.idCardNumber === "")
        return this.$Message.error("8.身份证号不能为空");
      if (this.form.address === "")
        return this.$Message.error("9.现住址不能为空");
      if (this.form.patient === "")
        return this.$Message.error(
          "10.本人及家庭成员是否为新冠肺炎确诊病人或疑似病人不能为空"
        );
      if (this.form.isolation === "")
        return this.$Message.error(
          "11.本人及家庭是否曾被要求隔离医学观察不能为空"
        );
      if (this.form.abroad === "")
        return this.$Message.error("12.是否曾出国或出境不能为空");
      if (this.form.importantArea === "")
        return this.$Message.error(
          "13.是否到过国内重点地区（和中高风险地区）不能为空"
        );
      if (this.form.contactPatient === "")
        return this.$Message.error(
          "14.是否接触过来自重点地区（和中高风险地区）或其他有本地病例持续传播地区的发热或有呼吸道症状患者不能为空"
        );
      if (this.form.aroundPatient === "")
        return this.$Message.error(
          "15.周围人群中有无2人及以上出现发热、干咳等症状或接触过新冠肺炎患者不能为空"
        );
      if (this.form.familySymptom === "")
        return this.$Message.error(
          "16.家人/同住人员有无发热、干咳等症状不能为空"
        );

      if (!/^[1][0-9]{10}$/.test(this.form.parentPhone))
        return this.$Message.error("家长联系电话不正确");
      if (!/(^\d{15}$)|(^\d{17}([0-9]|X)$)/.test(this.form.idCardNumber))
        return this.$Message.error("身份证号不正确");
