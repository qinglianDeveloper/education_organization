<!-- 新增积分商品 -->
<template>
  <div class="integral-add" style="overflow: auto">
    <Row style="overflow: auto">
      ...
      <div style="display:flex; justify-content:center;width:900px">
        <Button type="primary" @click="handleSave('integralForm')" style="margin-right:10px">保存</Button>
  ...
  methods: {
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          if (this.id == "add") {
            ...
            console.log(this.integralForm);
            if (this.integralForm.productDetail == "null") {
              this.integralForm.productDetail = "";
            }
            updateCreditDetail(this.integralForm).then(res => {
              if (res.code == 200) {
    ...
    handleRemove(name, index) {
      if (name == "main") {
        ...
        let urls = this.integralForm.bannerUrls.split(",");
        urls.splice(index, 1);
        this.integralForm.bannerUrls = urls.toString();
        console.log(this.integralForm.bannerUrls);
        // console.log(this.bannerUrls);
      }
    }
  }
