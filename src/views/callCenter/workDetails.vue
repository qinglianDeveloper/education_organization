      ...
      <Row class="header" type="flex" justify="space-between">
        ...
        <div class="btn-group">
          <Button
            ...
            size="small"
            @click="handleAllot"
            v-show="workForm.status=='DISALLOW'"
          >分配</Button>
          <Button
            type="default"
            size="small"
            @click="handleDispose"
            v-show="workForm.status=='DISAFFECT'"
          >受理</Button>
          <Button
            type="default"
            size="small"
            @click="handleTransfer"
            v-show="workForm.status=='ACCEPT'"
          >转交</Button>
          <Button
            type="default"
            size="small"
        ...
        <Tabs type="card">
          ...
          <TabPane label="备注" name="name5">
            <Row style="margin-bottom:6px">
              <Button type="primary" size="small" @click="addRemark">新增备注</Button>
            </Row>
            <Table ref="table" :columns="columns" :data="remarkData" border>
              <template slot-scope="{ row }" slot="accessory">
                <Button type="default" size="small" v-show="row.remarkUrl?true:false">
                  <a :href="row.remarkUrl">下载</a>
                </Button>
                <Button type="default" size="small" disabled v-show="row.remarkUrl?false:true">无附件</Button>
              </template>
          ...
          <TabPane label="验收报告" name="name6">
            <Form :label-width="100" ref="report" :model="report" :rules="reportRules">
              ...
              <FormItem label="附件：" prop="reportUrl">
                <!-- 下载 -->
            ...
            <Row>
              <Button @click="handleReport" type="primary" v-show="isResport">提交报告</Button>
            </Row>
          </TabPane>
        </Tabs>
      </Row>
...
export default {
  ...
  data() {
    return {
      // order:
      //   '[{"attributeName":"规格","attributeValue":"10台实例以内"},{"attributeName":"长度","attributeValue":"长"},{"attributeName":"高度","attributeValue":"高"}]',
      // 操作历史
      loading: false,
      tableHeight: 0,
      logColumns: [
  ...
  methods: {
    ...
    pageHeight() {
      ...
      let pageHeight = Number(
        document.getElementsByClassName("work-details")[0].clientHeight
      );
      ...
      if (from == "workOrder") {
        getOrderInfo({ id }).then(res => {
          ...
          if (res.code == 200) {
            ...
            let jso = JSON.parse(this.workForm.feildsItem);
            this.feildsItem = jso;
    ...
    handleEdit() {
      ...
      if (this.edit) {
        ...
        this.workForm.feildsItem = JSON.stringify(this.feildsItem);
        // console.log(this.workForm);
        this.$Modal.confirm({
          title: "确认修改",
          content: "您确认要保存修改的工单？",
