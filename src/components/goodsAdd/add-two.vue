          ...
          <div class="select-name">
            <Form ref="cardName" style="display:flex;" :rules="rules" :model="card">
              ...
              <FormItem label="单价：" :label-width="70" style="margin-left:20px" prop="price">
                <Input type="number" placeholder="请输入单价" style="width:160px" v-model="card.price"></Input>
              </FormItem>
            </Form>
            <!-- <span>规格名称：</span> -->
            <!-- <div class="price" style="margin-left:30px">
              <span>单价：</span>
              <Input type="number" placeholder="请输入单价" style="width:200px" v-model="card.price"></Input>
            </div>-->
                ...
                <div v-if="item.status">
                  ...
                  >
                    ...
                    <FormItem style="width:40%" prop="ratio">
                      <Input v-model="item.ratio" placeholder="请输入..." type="number" />
                    </FormItem>
                    <FormItem>
                      <Button
                        type="error"
...
<script>
export default {
  props: { goods: Array, design: String, chDesign: String },
  data() {
    // 校验规格值长度
    const validateValue = (rule, value, callback) => {
      var reg = new RegExp("^.{15,}$");
      if (!reg.test(value)) {
        callback();
      } else {
        callback(new Error("不能超过15个字符"));
      }
    };
    return {
      cardForm: [
      ...
      attributeRules: {
        ...
        ratio: [{ required: true, message: "请填写规格系数", trigger: "blur" }]
      },
      modal: false,
      modalTitle: "",
      show: false,
  ...
  methods: {
    addAttribute() {
      ...
      let y = this.goods[this.goods.length - 1].big;
      let ascll = y;
      // console.log(y);
    ...
    addList(i) {
      ...
      let y = list[leng - 1].small;

      let ascll = String.fromCharCode(y);
      // console.log(y, ascll);
